...
项目核心功能：
- 模拟图书馆环境（支持自定义网格大小，默认20x20网格，400个座位）
...
环境包含必要的Python包，如Flask、Flask-CORS、OpenAI、httpx、pydantic等。
...
目前项目架构已建立，包含完整的座位管理系统、学生系统、LLM客户端和提示词系统。测试用例已实现，特别是座位系统、学生系统和交互测试覆盖了各种场景。图书馆系统已实现基本功能，包括座位初始化、学生初始化和管理功能。模拟框架提供了交互式命令行界面，支持参数调节和实时监控。
